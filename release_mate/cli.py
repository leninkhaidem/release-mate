...
def init(
    ...
    try:
        ...
        config_file = _validate_config_file(config.project_id, config)
...
def _validate_config_file(project_id, config):
...
@cli.command()
@click.argument('project-id', required=False, shell_complete=project_id_completion)
